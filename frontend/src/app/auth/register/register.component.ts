  ...
  template: `
    <div class="container flex justify-center items-center" style="min-height: 80vh">
      <div class="card" style="padding: 2rem; width: 100%; max-width: 400px;">
        <h2 class="text-center mb-4">Register for HealCare</h2>
        <form (ngSubmit)="onSubmit()">
          <div class="mb-4">
            <label class="block mb-2">Full Name</label>
            <input type="text" class="search-input w-full border" [(ngModel)]="name" name="name" required style="border: 1px solid var(--border-light)">
          </div>
          <div class="mb-4">
          ...
          <div class="mb-4">
            ...
            <select class="search-input w-full border" [(ngModel)]="role" name="role" required style="border: 1px solid var(--border-light); background: var(--bg-card)">
...
export class RegisterComponent {
  ...
  onSubmit() {
    this.authService.register({ name: this.name, email: this.email, password: this.password, role: this.role }).subscribe({
      ...
      error: (err) => {
        this.error = 'Registration failed. User may exist.';
      }
    });
  }
}
